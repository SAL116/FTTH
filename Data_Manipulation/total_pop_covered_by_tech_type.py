...
allstates.to_csv("/home/analysis/Final/poplulation/{0}/techtype_each_state.csv".format(year[:7]),index=False)
out = allstates.groupby(['TechType','HighSpeed'])['population'].sum().reset_index()
out.to_csv("/home/analysis/Final/ISP/poplulation/{0}/techtype_pop.csv".format(year[:7]),index=False)